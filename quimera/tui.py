...
class BackgroundTextEditor(App):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Horizontal(classes="top-status-bar"):
            yield Static(f"[bold]BLOCKER[/bold]: {self.current_blocker}", classes="status-widget", id="status-widget")
            yield Static(f"[bold]STATUS[/bold]: {self.main_task_status}", classes="status-widget", id="task-widget")  
    ...
    def _update_directory_tree(self) -> None:
        ...
        try:
            dir_path = str(Path(self.directory_tree_path).resolve())  # Ensure path is valid and absolute
    ...
    def _update_time_display(self) -> None:
        ...
        time_widget.update(f"[bold]TARGET[/bold]: {self.network_information}") #, [bold]TIME[/bold]: {self._get_elapsed_time()}")
